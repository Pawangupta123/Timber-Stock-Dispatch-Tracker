...
export default function NewOutwardPage() {
  ...
  useEffect(() => {
    async function loadMasterData() {
      ...
      if (partyRes.error) toast.error('Failed to load receivers: ' + partyRes.error.message)
  ...
  async function handleSave() {
    ...
    if (validSections.length === 0 || total === 0) {
    ...
    toast.success(`Challan #${challanNumber} saved! ${validSections.length} section(s) — Total: ${total} CFT`)
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={form.receiver_id} onValueChange={v => setForm({ ...form, receiver_id: v ?? '' })}>
                ...
                <SelectContent>
                  {receivers.map(r => <SelectItem key={r.id} value={String(r.id)}>{r.name}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-2">
