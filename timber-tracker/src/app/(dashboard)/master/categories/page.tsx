...
export default function CategoriesPage() {
  ...
  async function handleAdd() {
    ...
    else { toast.success('Category added!'); setNewName(''); load() }
    setLoading(false)
  }
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card>
          ...
          <CardContent>
            <div className="flex gap-2">
              <Input
                placeholder="e.g. Mango, Pine, Teak..."
                value={newName}
                onChange={(e) => setNewName(e.target.value)}
                onKeyDown={(e) => e.key === 'Enter' && handleAdd()}
              />
              <Button onClick={handleAdd} disabled={loading} className="bg-green-600 hover:bg-green-700">
        ...
        <Card>
          <CardHeader><CardTitle className="text-base">All Categories ({items.length})</CardTitle></CardHeader>
          <CardContent className="space-y-2">
            ...
            {items.length === 0 && <p className="text-gray-400 text-center py-4">No categories yet</p>}
          </CardContent>
        </Card>
      </div>
    </div>
